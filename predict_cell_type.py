...
biomarkers = [col for col in data.columns if col not in ['CELL_ID', 'CLUSTER_LABEL', 'ACQUISITION_ID']]
...
y = label_encoder.fit_transform(data['CLUSTER_LABEL'].to_pandas())  # convert to pandas for label encoding
...
num_classes = len(data["CLUSTER_LABEL"].unique())
